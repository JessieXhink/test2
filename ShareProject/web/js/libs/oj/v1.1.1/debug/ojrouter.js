...
oj.Router = function(key, parentRouter) {
    ...
    * @export
    */
   oj.RouterState = function (id, options) {
      options = options || {};
      ...
      if ((typeof(options['isDefault']) === 'boolean') && options['isDefault']) {
         router._defaultStateId = id;
      }

      /**
       * This property is used when the router states are ojModule mames.
    ...
    * @return {!Promise.<{hasChanged: boolean}>} A Promise that resolves when the router is done
   ...
   oj.RouterState.prototype.isCurrent = function() {
      return router._stateId() === this.id;
   };
   ...
   /**
    * The state id of the default state for this router.
    * If undefined the router will start without a state selected.
...
Object.defineProperties(oj.Router.prototype, {
   ...
   'defaultStateId': { get: function () { return this._defaultStateId; }, enumerable: true },
...
oj.Router.prototype.configure = function(option) {
   ...
   if (typeof option === 'function') {
      ...
      Object.keys(option).forEach(function(key) {
         this._states.push(new oj.RouterState(key, option[key]));
      }, this);
   }
...
oj.Router.prototype.go = function(stateId) {
   oj.Assert.assertStringOrNull(stateId);
