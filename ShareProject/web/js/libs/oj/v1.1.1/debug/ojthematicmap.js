...
oj.__registerWidget('oj.ojThematicMap', $['oj']['dvtBaseComponent'],
  {
    ...
    _Render: function() {
      ...
      if (areaLayers) {
        for (var i = 0; i < areaLayers.length; i++) {
          ...
          if (areaDataLayer) {
            var renderer = areaDataLayer['renderer'];
            if (renderer) {
              var data = areaDataLayer['markers'];
              areaDataLayer['_components'] = this._stampDataLayerChildren(data, renderer);
            }
          }
        }
      }
      ...
      if (pointDataLayers) {
        for (var i = 0; i < pointDataLayers.length; i++) {
          ...
          if (pointDataLayer) {
            var renderer = pointDataLayer['renderer'];
            if (renderer) {
              var data = pointDataLayer['markers'];
              pointDataLayer['_components'] = this._stampDataLayerChildren(data, renderer);
            }
          }
        }
      }
    ...
    /**
     * Calls a custom renderer function to stamp out children in a data layer
     * @param {Array} data The array of data to stamp children with
     * @returns {Array} The arrray of dvt components
     * @private
     */
    _stampDataLayerChildren: function(data, renderer) {
      var components = [];
      for (var i=0; i < data.length; i++) {
        // Currently we only support stamped data visualizations and we inline
        // them in the parent thematic map's svg element by sharing the the DvtContext
        // object which are saving as a private variable on the dummy div.
        // In the future this may change with the support and use of foreign objects.
        var dummyDiv = document.createElement("div");
        dummyDiv.style.display = "none";
        dummyDiv._dvtcontext = this._context;
        ...
        var elem = renderer.call(this, context);
        // If no element is returned, check dummyDiv for stamped div
        if (!elem)
          elem = dummyDiv.children[0];
        var dvtComponent = this._GetDvtComponent(elem);
        if (dvtComponent) {
          components.push({
            'id': data[i]['id'],
 ...
 * 
 * <ul>
 * <li><b>dataLayer</b>: The id of the data layer.</li>
 * <li><b>index</b>: The index of the area within the specified data layer.</li>
 * </ul>
 * 
 * @ojsubid
 * @member
 * @name oj-thematicmap-area
 * @memberof oj.ojThematicMap
 * @instance
 * 
