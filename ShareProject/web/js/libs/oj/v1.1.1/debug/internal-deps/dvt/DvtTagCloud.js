...
// TODO LEGEND &  HIDESHOWBHEAVIOR
// TODO ADF CSS
// TODO ADF DND
// TODO ADF POPUP/CONTEXT MENU
// TODO ADF ACTION
...
DvtTagCloud.prototype.Init = function(context, callback, callbackObj) {
  ...
  this.EventManager = new DvtTagCloudEventManager(this, context, this.__dispatchEvent, this);
...
DvtTagCloud.prototype.select = function(selection) {
  // TODO AMDAI: This has not been tested. Please test and remove this comment.
  // Update the options
...
DvtTagCloudDefaults.VERSION_1 = {
  ...
  'styleDefaults': {
    ...
    'style' : "font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; color: #333333;"
  }
};
/**
...
DvtTagCloudRenderer._renderEmptyText = function(tagCloud, container, availSpace) {
  ...
  DvtTextUtils.renderEmptyText(container, emptyTextStr,
      ...
      tagCloud.EventManager, new DvtCSSStyle(options['styleDefaults']['style']));
};
...
DvtTagCloudRenderer._renderItems = function(tagCloud, container, availSpace) {
  ...
  for (var i = 0; i < dataItems.length; i++) {
    ...
    var style = new DvtCSSStyle(options['styleDefaults']['style']);
  ...
  if (items.length > 0) {
    if (options['layout'] === 'cloud')
      DvtTagCloudLayoutUtils.cloudLayout(availSpace.w, availSpace.h, items);
    else
      DvtTagCloudLayoutUtils.rectangleLayout(availSpace.w, availSpace.h, items, DvtAgent.isRightToLeft(tagCloud.getCtx()));
  }
  return items;
};
/**
 ...
 * @param {Array} items
 */
DvtTagCloudLayoutUtils.cloudLayout = function(width, height, items) {
  var positions = [];
  var steps = 180;
  var xrStep = 10 / steps;
  var yrStep = 10 / steps;
  ...
  var scale = Math.max(bounds.w / width, bounds.h / height);
  var cx = bounds.x + bounds.w / 2;
  var cy = bounds.y + bounds.h / 2;
  for (var j = 0; j < items.length; j++) {
    var tag = items[j];
    tag.setX((tag.getX() / scale) + (width / 2 - cx / scale));
    tag.setY((tag.getY() / scale) + (height / 2 - cy / scale));
 ...
 */
DvtTagCloudLayoutUtils.rectangleLayout = function(width, height, items, isBidi) {
  var arDims = [];
  var maxWidth = 0;
  var maxHeight = 0;
  var maxFontSize = 0;
  ...
  var minScale = 0;
  var maxScale = (width - 2 * margin) / maxWidth; // This is the max scale that can be applied before the longest word will no longer fit
  var scale;
  var arLines;
  while (maxScale - minScale > .001) {
    scale = (minScale + maxScale) / 2;
    arLines = DvtTagCloudLayoutUtils._calculateLineBreaks(arDims, (width - 2 * margin) / scale);
  ...
  for (var i = 0; i < arLines.length - 1; i++) {
    ...
    if (lineEnd - lineStart > 1) {
      ...
      hPadding = (width - 2 * margin - lineWidth) / (lineEnd - lineStart - 1);
      if (i == arLines.length - 2) {
        ...
        if (flowPadding < hPadding && (lineWidth + (lineEnd - lineStart) * flowPadding) < .9 * (width - 2 * margin)) {
    ...
    var curX = isBidi ? width - margin : margin;
    for (var j = lineStart; j < lineEnd; j++) {
      var tag = items[j];
      var fontSize = parseFloat(tag.getCSSStyle().getStyle(DvtCSSStyle.FONT_SIZE));
      tag.setFontSize(fontSize * scale);
