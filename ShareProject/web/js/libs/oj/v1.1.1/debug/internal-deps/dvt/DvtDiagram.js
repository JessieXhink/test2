...
DvtBaseDiagram.prototype.Init = function(context, callback, callbackObj) {
  ...
  //FIX BUG 16776435: force text to scale linearly
  ...
  this._initAnim;
  //FIX BUG 14813478: flag used to disable the init anim on PPR
...
DvtBaseDiagram.prototype.CreateEmptyText = function(text) {
  return DvtTextUtils.renderEmptyText(this, text, new DvtRectangle(0, 0, this.getWidth(), this.getHeight()), this.__getEventManager());
};
...
DvtBaseDiagram.prototype.CreateLayoutContextNode = function(node, layout) {
  ...
  if (node.isDisclosed()) {
    ...
    nc.SetContainerPaddingObj(node.getContainerPadding());
  }
...
DvtBaseDiagram.prototype.ApplyLayoutContext = function(layoutContext, animator, bSaveOffset) {
  //FIX BUG 14823204: apply container padding that was set while laying out the container's children
  ...
  if (topContainerPadding) {
    ...
    if (containerId) {
      ...
      if (containerNode) {
        containerNode.setContainerPadding(topContainerPadding['top'], topContainerPadding['right'], topContainerPadding['bottom'], topContainerPadding['left'], animator);
      }
    }
  }
  ...
  for (var ni = 0; ni < layoutContext.getNodeCount(); ni++) {
    ...
    if (node.isDisclosed()) {
      ...
      if (containerPadding) {
        node.setContainerPadding(containerPadding['top'], containerPadding['right'], containerPadding['bottom'], containerPadding['left'], animator);
      }
    }
  }
  if (!this._bCrossedZoomThreshold) {
    for (var li = 0; li < layoutContext.getLinkCount(); li++) {
      ...
      var points = lc.getPoints();
      if (points) {
        //FIX BUG 16211930: turn list of points into path commands compatible for animating
        ...
        if (animator && link.getPoints()) {
          if (translatedPoints.length > 0 && isNaN(translatedPoints[0])) {
            //FIX BUG 16211930: transition between old and new paths
...
//Bug# 16776956 - HTML5 HV: ROOT NODE'S X COORDINATE CHANGED AFTER EXPAND/COLLAPSE
/**
 * Gets layout offset
 * @param {number} x x coordinate
 * @param {number} y y coordinate
...
DvtBaseDiagramLink.prototype.setPoints = function(points) {
  ...
  //FIX BUG 15867125: need to update the hit detection underlay when animating
  //a link
...
DvtDiagram.prototype.RenderComponentInternal = function(animator) {
  ...
  if (!emptyDiagram) {
    this.addChild(this._oldPanZoomCanvas); // the child is going to be removed by  _processContent() function or layout failure function
    var res = this.layout(animator);
    var thisRef = this;
    res.then(
        function() {thisRef._processContent(animator, emptyDiagram);}, //success
        function() {thisRef.removeChild(thisRef._oldPanZoomCanvas);thisRef._oldPanZoomCanvas = null;thisRef._bRendered = true;}   //failure
    );
  }
...
DvtDiagram.prototype._fitContent = function(animator) {
  ...
  if (!this._bRendered) {
    ...
    if (!this.IsResize()) {
      //BUG FIX 16695081: don't override a viewport returned from the layout engine
  ...
  else if (this.IsResize() && !this.IsLayoutViewport()) {
    ...
    if (!(this.IsPanningEnabled() && this.IsZoomingEnabled())) {
      pzc.setZoomToFitEnabled(true);
      pzc.zoomToFit(null, fitBounds);
      pzc.setZoomToFitEnabled(this.IsZoomingEnabled());
    }
    else {
      pzc.center(null, viewBounds);
    }
  }
};
...
/*
    ...
    DvtDiagramLinkConnectorUtil.js

    MODIFIED    (MM/DD/YY)
    lmolesky	05/07/12 - Refine return value from getConnectorOffset()
    lmolesky	04/13/12 - Added arrow styles, circle, rectangle.
    bglazer	06/11/11 - Created
...
DvtDiagramLinkUnderlay.prototype.setStroke = function(stroke, strokeOffset) {
  this._stroke = stroke;
  // Bug 15868719 - use non-scaling stroke on the link on older devices with iOS and Safari
  if (DvtAgent.isTouchDevice() && DvtAgent.isBrowserSafari() && DvtAgent.getDevicePixelRatio() == 1)
    stroke.setFixedWidth(true);
  this._underlay.setStroke(stroke);
...
DvtDiagramNode.prototype.getLabelBounds = function() {
  //BUG FIX 14186917: return saved label dimensions, because getting
