...
DvtAxisRenderer._renderLabelsHoriz = function(axis, axisInfo, availSpace) {
  ...
  while (labels) {
    ...
    for (var i = 0; i < labels.length; i++) {
      ...
      var datatip = axisInfo.getDatatip(i, levelIdx);
      ...
      axis.__getEventManager().associate(label, new DvtAxisObjPeer(label, group, action, drillable, tooltip, datatip, params));
...
DvtAxisRenderer._renderLabelsTangent = function(axis, axisInfo, availSpace) {
  ...
  for (var i = 0; i < labels.length; i++) {
    ...
    if (DvtTextUtils.fitText(label, maxWidth, maxHeight, axis)) { // truncation
      ...
      // support for categorical axis tooltip and datatip
      var datatip = axisInfo.getDatatip(i);
      var tooltip = label.getUntruncatedTextString();
      // action support
 ...
 */
var DvtAxisObjPeer = function(label, group, action, drillable, tooltip, datatip, params) {
  this.Init(label, group, action, drillable, tooltip, datatip, params);
};
 ...
 * @param {object=} params Optional object containing additional parameters for use by component.
 */
DvtAxisObjPeer.prototype.Init = function(label, group, action, drillable, tooltip, datatip, params) {
...
DvtAxisObjPeer.prototype.getAriaLabel = function() {
  if (this.getDatatip() != null) //axis label has shortDesc defined
    return DvtDisplayable.generateAriaLabel(this.getDatatip(), null);
};
/**
...
DvtTimeAxisInfo.prototype.Init = function(context, options, availSpace) {
  ...
  this._skipGaps = this._timeAxisType == 'skipGaps';
