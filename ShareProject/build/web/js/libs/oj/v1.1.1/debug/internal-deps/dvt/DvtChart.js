...
DvtSelectableWedge.prototype._makeWedgePath = function(inset) {
  ...
  var distanceToStartPoint = angleExtentRads > 0 ? Math.abs(dataItemGaps / Math.sin(angleExtentRads / 2)) : 0;

  // cap distanceToStartPoint because tiny wedges risk distance being so big it gets drawn outside of the pie
  if (distanceToStartPoint > rx * 0.5)
    distanceToStartPoint = rx * 0.5;

  var startPointX = this._cx + Math.cos(centerLineAngle) * distanceToStartPoint;
  var startPointY = this._cy + Math.sin(centerLineAngle) * distanceToStartPoint;
...
DvtFunnelSlice.prototype._getSliceLabel = function(sliceBounds, barBounds) {
  ...
  var labelStyleArray = [this._chart.getOptions()['styleDefaults']['dataLabelStyle'], this._chart.getOptions()['styleDefaults']['sliceLabelStyle'],
...
DvtChartAutomation.prototype.getDomElementForSubId = function(subId) {
  // TOOLTIP
  if (subId == 'tooltip') {
    var tooltipManager = this._chart.getCtx().getTooltipManager(DvtChartTooltipUtils.isDataCursorEnabled(this._chart) ? DvtDataCursor.TOOLTIP_ID : null);
    var tooltipElem = tooltipManager.getTooltipElem();
...
DvtChartStyleUtils.getDataLabelPosition = function(chart, seriesIndex, groupIndex, type) {
  ...
  if (DvtChartTypeUtils.isFunnel(chart)) {
    return (position == 'none') ? 'none' : 'center';
  }
  // Bar series
...
DvtPieLabelUtils._createLabel = function(slice, isInside) {
  ...
  var labelStyleArray = [styleDefaults['dataLabelStyle'], styleDefaults['sliceLabelStyle']];
