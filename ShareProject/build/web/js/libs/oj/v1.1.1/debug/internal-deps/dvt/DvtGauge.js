...
DvtLedGaugeRenderer._renderShape = function(gauge, container, bounds) {
  ...
  if (type == 'rectangle') {
  ...
  else if (type == 'star' || type == 'triangle' || type == 'arrow' || type == 'human') {
    if (type == 'star')
    ...
    if (isSkyros && type == 'triangle' || isSkyros && type == 'arrow' || type == 'human') {
...
DvtLedGaugeRenderer._getLabelBounds = function(gauge, container, bounds) {
  ...
  else if (type == 'circle' || type == 'human') {
    newX += minDim * .15;
    newY += minDim * .15;
    newWidth -= minDim * .3;
    newHeight -= minDim * .3;
...
DvtStatusMeterGaugeRenderer._renderCircularGauge = function(gauge, container, bounds) {
  ...
  if (thresholds && options['plotArea']['rendered'] != 'off' && options['thresholdDisplay'] == 'all') {
    ...
    DvtStatusMeterGaugeRenderer._drawCircularArc(gauge, container, bounds, startAngleRads, angleExtentRads, innerRadiusLength, outerRadius, thresholdColor, true);
  }
...
/** @private **/
DvtRatingGaugeRenderer._VALID_SHAPES = ['circle', 'diamond', 'rectangle', 'star', 'triangle', 'human'];
/**
 * Renders the gauge in the specified area.
...
DvtRatingGaugeEventManager.prototype.OnMouseMove = function(event) {
  ...
  var coords = this.GetRelativePosition(event.pageX, event.pageY);
  if (this._gauge.getOptions()['readOnly'] === false && !this.isEditing &&
