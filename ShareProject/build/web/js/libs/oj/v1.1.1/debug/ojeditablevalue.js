...
oj.PopupMessagingStrategyUtils.buildHintHtml = function (document, selector, hintText, htmlAllowed, formControlSelectors)
{
  ...
  if (hintText)
  {
    ...
    // @HTMLUpdateOK
    jTitleDom.append(oj.PopupMessagingStrategyUtils._getTextDom(document, hintText, htmlAllowed));
  }
  ...
  function (document, summary, detail, severityLevel, addSeverityClass)
{
  ...
  if (detail)
  {
    ...
    // @HTMLUpdateOK
    $msgDetail.addClass(oj.PopupMessagingStrategyUtils._SELECTOR_MESSAGE_DETAIL).append(detailDom);
...
oj.__registerWidget('oj.editableValue', $['oj']['baseComponent'], 
{
  ...
  options: 
  {
     ...
     * anchor's target is this source. The default value is null.
     * 
     * @expose 
     * @memberof oj.editableValue
     * @instance
...
oj.__registerWidget("oj._ojLabel", $['oj']['baseComponent'],
{
  ...
  */
  ...
  _drawOnCreate : function ()
  {
    ...
    // @HTMLUpdateOK
    this.uiLabel = this.element.wrap(this._createRootDomElement())
    ...
    if (requiredOpt)
    {
      ...
      if (ariaRequiredUnsupported)
        describedByDom.append(this._createRequiredIconDomElement());  // @HTMLUpdateOK  
      else
  ...
  {
    var rootAttributes = this.options.rootAttributes, inputLabelClass, rootDomNode;
    ...
    {
      rootDomNode.addClass(inputLabelClass);
    }
    return rootDomNode;
  },
  /** 
  ...
  _createHelpIconAnchorDomElement : function (helpDef, source)
  {
    ...
    if (this._needsHelpIcon())
    {
      ...
      helpIconAnchor = 
        ...
        this.document[0] );
      if (source)
        helpIconAnchor.attr("href", source);

      if (helpDef)
       helpIconAnchor.attr("title", helpDef);
      else
       helpIconAnchor.attr("title", this.getTranslatedString(this._BUNDLE_KEY._TOOLTIP_HELP));
   ...
   _attachHelpDefToIconAnchor : function ()
  {
    ...
    if (!this._helpDefPopupDivId)
    {
      ...
      $("body").append(helpDefPopupDiv); // @HTMLUpdateOK
    }
    else
    {
      // Find the div with the id, and then update the text of it.
      helpDefPopupDiv = $('#' + this._helpDefPopupDivId);
      if (helpDefPopupDiv)
      {
        helpDefPopupDiv.text(helpDefText);
      }
    ...
    else
    {
      ...
      helpDefPopupDiv.ojPopup({"position": position});  
  },
    /** 
  ...
  _refreshHelp : function ()
  {
    ...
    if (needsHelpIcon != null && describedBySpan.length === 0)
    {
      // no describedBySpan, so we need to create one
      describedBySpan = this._createDescribedBySpan();

    }
    else if (needsHelpIcon == null && describedBySpan.length > 0)
    {
    ...
    if (this.options.required)
    {
      ...
      if (requiredDom.length === 0)
      {
        ...
        if (this.options.ariaRequiredUnsupported)
        {
          ...
          // @HTMLUpdateOK
          describedBySpan.append(this._createRequiredIconDomElement());
        }
        else
        {
  ...
  getNodeBySubId: function(locator)
  {
    ...
    if (!node)
    {
      ...
      if (subId === "oj-label-help-icon-anchor") {
        node = this.widget().find(".oj-label-help-icon-anchor")[0];
      }
